...
//		$this->setTitle();
   ...
   /**
	...
	protected function initUser(){
		...
		//获取AppUserInfo数据
		if ( unserialize($_SESSION['AppUserInfo']) == '' || 
			!$_SESSION['userInfo']['is_init'] || 
			!$_SESSION['userInfo']['is_active'] ) {
			$userInfo	=	model('User')->find($this->mid);
			$_SESSION['AppUserInfo']	=	serialize($userInfo);
			$_SESSION['userInfo']		=	$userInfo;
		}

		//赋值当前用户
		$this->user	=	unserialize($_SESSION['AppUserInfo']);
		global $ts;
		$ts['user']	=	$this->user;

		//$this->assign('notifyNum',M('notify')->where('touid='.$this->mid.' AND isread=0')->count());
	...
	protected function initUserApp() {
		...
		$sql = "SELECT a.* FROM  {$prefix}app AS a LEFT JOIN {$prefix}user_app AS u ON a.app_id = u.app_id " . 
			   "WHERE a.status = '1' OR ( a.status = '2' AND u.uid = '$mid' ) GROUP BY `app_id` " .
			   "ORDER BY a.status ASC,a.display_order ASC,a.app_id ASC,u.display_order ASC";
		$res = M('')->query($sql);
		...
		if (!$has_installed && model('App')->isAppAdmin(APP_NAME, $ts['_mod']))
			$has_installed = true;
		if (!$has_installed) {
			$this->assign('jumpUrl', U('home/Index/install',array('app_name'=>APP_NAME)));
			$this->error('您尚未添加应用，请先添加');
		}
		
		// 漫游应用
		if ($ts['site']['my_status']) {
			$default = model('Myop')->getAllDefaultApp();
...
//		if(is_string($input)){
//			$this->site['title']	=	$input;
//		}else if(is_array($input)){
//			foreach($input as $k=>$v){
//				$this->site[$k]	=	$v;
//			}
//		}
//		$this->view->assign('xsite',$this->site);
	}
