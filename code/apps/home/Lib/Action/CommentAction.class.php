<?php
    ...
    class CommentAction extends Action{
        ...
        public function doQuickReply() {
        	$_POST['to_id'] 	= intval($_POST['to_id']);
        	$_POST['comment']	= t($_POST['comment_content']);
        	...
        	// 插入新数据
        	$former_comment['to_uid']	= $former_comment['uid'];
        	$former_comment['uid']		= $this->mid;
        	$former_comment['comment']	= $_POST['comment'];
        	$former_comment['cTime']	= time();
        	$former_comment['toId']		= $former_comment['id'];
        	$former_comment['status']	= 0;
        	
        	unset($former_comment['id']);
        	$res = M('comment')->add($former_comment);
        	...
        	if($res) {
        		...
        		if ( intval($_POST['with_new_weibo']) ) {
        			$from_data = array('app_type'=>'local_app', 'app_name'=>$former_comment['type'], 'title'=>$callback_data['title'], 'url'=>$callback_data['url']);
        			$from_data = serialize($from_data);
					D('Weibo','weibo')->publish($this->mid, array('content'=>$_POST['comment']), 0, 0, '', '', $from_data);
        		}
...
*/
				...
				$result['data']['ctime']    		= '刚刚';
				$result['data']['uname']    		= getUserName($this->mid);
				$result['data']['comment']			= formatComment($_POST['comment']);
				$result['data']['id']				= $res;
