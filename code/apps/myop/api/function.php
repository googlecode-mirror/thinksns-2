...
function getCurrentUser() {
	...
	if($_session_user_id){
		...
		$cookieId = explode( '.', base64_decode($_cookie_user) );
...
function U($main, $param = array()) {
	...
	$main	= explode('/', $main);
	$app	= $main[0];
	$mod	= $main[1] ? $main[1] : 'Index';
	$act	= $main[2] ? $main[2] : 'index';
	$url	= SITE_URL . "/index.php?app=$app&mod=$mod&act=$act";
	if ( !empty($param) ) {
		if ( is_string($param) ) {
			$url .= "&$param";
...
function getUserFace($uid,$size='m'){
	...
	$userface = SITE_ROOT.'/data/uploads/avatar/'.$uid.'/'.$type;
	if(is_file($userface)){
